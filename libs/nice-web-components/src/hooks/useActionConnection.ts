...
import { ICoordinate, IPlacedReactionObjectMessage } from "@nice/nice-common";
  ...
  const communicator = useMemo(() => {
    return {
      ...
      updateObject(object: fabric.Rect) {
        ...
        if (!object.aCoords) {
          throw new Error("Object has no aCoords");
        }
...
function getCoordinatesForLine(
  fromPoint: fabric.Point,
